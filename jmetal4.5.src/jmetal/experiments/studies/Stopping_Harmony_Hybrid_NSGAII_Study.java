 ...
 * @author UsEr
 */
public class Stopping_Harmony_Hybrid_NSGAII_Study extends Experiment {
...
//            algorithm[1] = new Harmony_Hybrid_MOEAD_Settings(problemName).configure(parameters[1]);
            algorithm[0] = new Harmony_Hybrid_NSGAII_Settings(problemName).configure(parameters[0]);
            algorithm[1] = new Harmony_Hybrid_NSGAII_Settings(problemName).configure(parameters[1]);
//            algorithm[0] = new Harmony_MOEAD_Settings(problemName).configure(parameters[0]);
//            algorithm[1] = new Harmony_MOEAD_Settings(problemName).configure(parameters[1]);
...
//            algorithm[0] = new Harmony_MOEAD_Settings(problemName).updateSettings(algorithm[0], params1);
//            algorithm[1] = new Harmony_MOEAD_Settings(problemName).updateSettings(algorithm[1], params2);
            algorithm[0] = new Harmony_Hybrid_NSGAII_Settings(problemName).updateSettings(algorithm[0], params1);
            algorithm[1] = new Harmony_Hybrid_NSGAII_Settings(problemName).updateSettings(algorithm[1], params2);
//            algorithm[4] = new Harmony_NSGAII_Settings(problemName).updateSettings(algorithm[4], params);
    ...
    public static void main(String[] args) throws JMException, IOException {
        Stopping_Harmony_Hybrid_NSGAII_Study exp = new Stopping_Harmony_Hybrid_NSGAII_Study(); // exp = experiment

        exp.experimentName_ = "StoppingHarmonyHybridNSGAII";
