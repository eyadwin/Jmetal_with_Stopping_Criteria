...
public class All_MOEAD_Methods extends Experiment {
    ...
    public synchronized void algorithmSettings(String problemName,
        ...
        try {
            ...
            //Hybrid_MOEAD
            /*
            algorithm[15] = new H_MOEAD_Settings(problemName).configure(parameters[10]);
            algorithm[16] = new H_MOEAD_Settings(problemName).configure(parameters[11]);
            algorithm[17] = new H_MOEAD_Settings(problemName).configure(parameters[12]);
            algorithm[18] = new H_MOEAD_Settings(problemName).configure(parameters[13]);
            algorithm[19] = new H_MOEAD_Settings(problemName).configure(parameters[14]);
            */
            ...
            {
                algorithm[1+i] = new MOEAD_Settings(problemName).updateSettings(algorithm[1+i],params[i][0]);
                algorithm[6+i] = new Harmony_MOEAD_Settings(problemName).updateSettings(algorithm[6+i],params[i][1]);
                algorithm[11+i] = new Harmony_Hybrid_MOEAD_Settings(problemName).updateSettings(algorithm[11+i],params[i][2]);
                //algorithm[16+X] = new H_MOEAD_Settings(problemName).updateSettings(algorithm[16+i],params[i][3]);
