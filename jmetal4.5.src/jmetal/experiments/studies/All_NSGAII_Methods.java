...
public class All_NSGAII_Methods extends Experiment {
    ...
    public synchronized void algorithmSettings(String problemName,
        ...
        try {
            ...
            algorithm[2] = new NSGAII_Settings(problemName).configure(parameters[2]);
            algorithm[3] = new NSGAII_Settings(problemName).configure(parameters[3]);
            algorithm[4] = new NSGAII_Settings(problemName).configure(parameters[4]);
            //Harmony_NSGAII
            algorithm[5] = new Harmony_NSGAII_Settings(problemName).configure(parameters[5]);
            algorithm[6] = new Harmony_NSGAII_Settings(problemName).configure(parameters[6]);
            algorithm[7] = new Harmony_NSGAII_Settings(problemName).configure(parameters[7]);
            algorithm[8] = new Harmony_NSGAII_Settings(problemName).configure(parameters[8]);
            algorithm[9] = new Harmony_NSGAII_Settings(problemName).configure(parameters[9]);
            //Hybrid_NSGAII
            algorithm[10] = new H_NSGAII_Settings(problemName).configure(parameters[10]);
            algorithm[11] = new H_NSGAII_Settings(problemName).configure(parameters[11]);
            algorithm[12] = new H_NSGAII_Settings(problemName).configure(parameters[12]);
            algorithm[13] = new H_NSGAII_Settings(problemName).configure(parameters[13]);
            algorithm[14] = new H_NSGAII_Settings(problemName).configure(parameters[14]);
            //Harmony_Hybrid_NSGAII
            algorithm[15] = new Harmony_Hybrid_NSGAII_Settings(problemName).configure(parameters[15]);
            algorithm[16] = new Harmony_Hybrid_NSGAII_Settings(problemName).configure(parameters[16]);
            algorithm[17] = new Harmony_Hybrid_NSGAII_Settings(problemName).configure(parameters[17]);
            algorithm[18] = new Harmony_Hybrid_NSGAII_Settings(problemName).configure(parameters[18]);
            algorithm[19] = new Harmony_Hybrid_NSGAII_Settings(problemName).configure(parameters[19]);
            ...
            for (int i = 0,X=0; i < params[0].length; i++,X+=5) 
            {
                algorithm[1+X] = new NSGAII_Settings(problemName).updateSettings(algorithm[1+X],params[0][i]);
                algorithm[2+X] = new Harmony_NSGAII_Settings(problemName).updateSettings(algorithm[2+X],params[1][i]);
                algorithm[3+X] = new H_NSGAII_Settings(problemName).updateSettings(algorithm[3+X],params[2][i]);
                algorithm[4+X] = new Harmony_Hybrid_NSGAII_Settings(problemName).updateSettings(algorithm[4+X],params[3][i]);
            }
    ...
    public static void main(String[] args) throws JMException, IOException {
        All_NSGAII_Methods exp = new All_NSGAII_Methods(); // exp = experiment
